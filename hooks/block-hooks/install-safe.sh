...
# Build keyword array for injection into hook script
keywords_array_items=""
for keyword in "${keywords[@]}"; do
  # Escape any double quotes in the keyword value
  escaped_keyword=$(printf '%s' "$keyword" | sed 's/"/\\"/g')
  keywords_array_items="${keywords_array_items} \"${escaped_keyword}\""
done

log "INFO" "Prepared ${#keywords[@]} keywords for injection (exact token matching)"
...
# Verify the hook script contains patterns
