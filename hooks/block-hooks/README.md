...
Ships with volume deletion protection enabled by default:
- `volumeDelete`, `deleteVolume`, `volume-delete`, `volume_volume`
...
**Important:** Keywords are explicit. If you add `dropDatabase`, it will only block commands containing exactly that string. Add variants you need (`drop-database`, `DROP_DATABASE`, etc.) manually to minimize false positives.
...
Removes all hooks and configuration files.
...
- **Hook script:** `~/.agent-hooks/block-hooks.sh` — the actual blocking logic (generated from keywords.txt)
- **Logs:** `~/.agent-hooks/agent-hooks.log` — all hook activity (ALLOW/BLOCK decisions)

### Detection logic

The hook reads the tool call payload from stdin and searches for exact substring matches against all keywords in `keywords.txt`.

**Default keywords** (volume operations):
- `volumeDelete`, `deleteVolume`, `volume-delete`, `volume_delete`
- `VOLUMEDELETE`, `DELETE-VOLUME`, `DELETE_VOLUME`, `VolumeDelete`

If any keyword is found in the payload: **block**. Otherwise: **allow**.
